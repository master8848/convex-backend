...
| Zig | ✅ **valid target** | Zig 0.16+ `build-exe -target wasm32-wasi -mexec-model=reactor` | Smallest possible guest: freestanding reactor module, **zero WASI imports** (only the 4 `env` host functions), 394 B echo / 837 B args-parsing; Zig 0.16 needs explicit `--export=__convex_run --export=__convex_functions`. Fixture `tests/fixtures/zig_guest` + e2e test; example `examples/wasm-guests/zig`. See `docs/zig-guest.md` |
